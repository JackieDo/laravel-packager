...
class RemovePackageCommand extends Command
{
     ...
     * @var string
     */
    protected $name = 'packager:remove';
     ...
     * @var string
     */
    protected $description = 'Remove an existing package with the given name.';
    ...
    public function fire()
    {
        ...
        if ($installed) {
            if (!$canUninstall) {
                $this->errorBlock('This package cannot be removed because it is required by another package.');
        ...
        if ($confirmed) {
            ...
            $this->write(' Removing the package...', 'info');

            $removed = $this->manager->remove($packageName);
            ...
            $this->comment(' OK');
            $this->successBlock('Your package has been removed successfully.');
    ...
    {
        return [
            ['uninstall', 'u', InputOption::VALUE_NONE, 'Request to uninstall package before removing.'],
        ];
    }
}
